...
{
    public class ExamModelTrial
    {
    }
    public class GenerateQuestionsTrial
    {
        ...
        public GenerateQuestionsTrial(int examid, string subjectids)
        {
            examId = examid;
            ...
            try
            {
                using (var db = new EXAMSPURDBEntities())
                {
                    ...
                    foreach (var item in subjectIds.Split(','))
                    {
                        ...
                        var sub2 = db.C_tblQuestionUTMETrial.Where(o => o.SubjectID == citem);
                        ...
                        for (int i = 0; i < 5; i++)
                        {
                            ...
                            {
                                var questiondetail = db.C_tblQuestionUTMETrial.Where(o => o.QuestionNumber == randomNumber && o.SubjectID == citem).FirstOrDefault();
                                if (questiondetail == null) goto ln;
                                if (questiondetail.HasImage >= 1)
                                {
                                    var questionwithImage = db.C_tblQuestionUTMETrial.Where(o => o.ImageId == questiondetail.ImageId && o.SubjectID == citem).OrderBy(o => o.QuestionNumber).ToList();
    ...
    public class CheckExamT
    {

        public Int64 ExamId { get; set; }
        public CheckExamT(Int64 examid)
        {
            this.ExamId = examid;
        }
    ...
    public class GetNextQuestionTrial
    {

        public Int64 SubjectId { get; set; }
        public Int64 QuestionNo { get; set; }
        public GetNextQuestionTrial(Int64 subjectid, Int64 questionno)
        {
        ...
        public object Question()
        {
            try
            {
                ...
                {
                    db.Configuration.ProxyCreationEnabled = false;
                    var model = db.C_tblQuestionUTMETrial.Where(o => o.SubjectID == SubjectId && o.QuestionNumber == QuestionNo).FirstOrDefault();
                    return model;
                }
    ...
    public class UpadateAnswersT
    {
        ...
        public UpadateAnswersT(Int64 subjectid, Int64 examtakenid, string answers, int curentTime)
        {
            this.SubjectId = subjectid;
    ...
    public class SubmitExamTrial
    {
        public Int64 ExamTakenId { get; set; }
        public SubmitExamTrial(long examtakenid)
        {
            this.ExamTakenId = examtakenid;
        }
    ...
    public class GetCorrectionTrial
    {

        public int SubjectId { get; set; }
        public Int64 ExamTakenid { get; set; }
        public GetCorrectionTrial(int subjectid, Int64 examtakenid)
        {
            this.ExamTakenid = examtakenid;
            this.SubjectId = subjectid;
        }
        public object getListOfCorrection()
        {
            try
            {
                using (var db = new EXAMSPURDBEntities())
                {
                    var questions = new QuestionForCoorrectionViewModelTrial();
                    ...
                    else
                    {
                        ...
                        var qlist = new List<C_tblQuestionUTMETrial>();

                        foreach (var item in list)
                        {
                            var qn =Convert.ToInt32(item);
                            var b = (C_tblQuestionUTMETrial)db.C_tblQuestionUTMETrial.Where(o => o.QuestionNumber == qn).FirstOrDefault();
                            qlist.Add(b);
    ...
    public class QuestionForCoorrectionViewModelTrial
    {
        public List<C_tblQuestionUTMETrial> correctionQ { get; set; }
